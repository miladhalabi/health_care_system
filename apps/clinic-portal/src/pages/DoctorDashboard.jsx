    ...
    return (
      <Layout>
        <Card className="p-20 text-center flex flex-col items-center">
           ...
           <h3 className="text-2xl font-black text-stone-900 mb-4">بوابة العيادة جاهزة</h3>
                ...
                <Card className="p-12 relative overflow-hidden group">
                   <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
                      ...
                      <div className="flex-1 text-center md:text-right">
                        ...
                        <h3 className="text-4xl font-black text-stone-900">{activePatient.patient.user.fullName}</h3>
                        <p className="text-stone-400 font-bold mt-2 uppercase tracking-widest text-sm">الرقم الوطني: {activePatient.patient.nationalId}</p>
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <Button variant="ghost" className="text-stone-400 font-bold">تأجيل المريض</Button>
                      </div>
                   </div>
                </Card>
              ) : (
                <Card className="p-24 border-2 border-dashed border-stone-200 text-center">
                   <div className="text-7xl mb-8 opacity-20">🏥</div>
                   <h3 className="text-2xl font-black text-stone-300 uppercase tracking-widest">العيادة جاهزة لاستقبال المراجع التالي</h3>
            ...
            <div className="animate-in fade-in zoom-in-95 duration-500">
               ...
               <Card className="sticky top-0 z-20 mb-8 p-6 border-b-4 border-primary shadow-lg flex flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-stone-900 leading-none">{activePatient.patient.user.fullName}</h3>
                      <p className="text-[10px] font-bold text-stone-400 mt-1 uppercase tracking-widest">رقم وطني: {activePatient.patient.nationalId}</p>
                    </div>
                  </div>
                  
                  <div className="flex gap-4">
                     <div className="text-center px-4 border-r border-stone-100">
                        <span className="text-[9px] font-black text-stone-400 uppercase block">زمرة الدم</span>
                        <span className="text-sm font-black text-primary">{activePatient.patient.bloodType || '--'}</span>
                     </div>
                     <div className="text-center px-4">
                        <span className="text-[9px] font-black text-stone-400 uppercase block">الحساسية</span>
                        <span className="text-sm font-black text-rose-500">{activePatient.patient.allergies || 'لا يوجد'}</span>
                     </div>
                  </div>

                  <Button variant="ghost" onClick={() => setIsEncountering(false)} className="text-stone-300 hover:text-stone-900 text-[10px]">إلغاء الجلسة</Button>
               </Card>

               <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                  <div className="space-y-8">
                    <Card className="p-8">
                       <h4 className="text-xs font-black text-stone-400 uppercase tracking-[0.2em] mb-6">التشخيص السريري</h4>
                       <div className="space-y-6">
                          <div className="form-control">
                             <label className="label py-1"><span className="label-text font-bold text-stone-500 text-xs text-right w-full">الأعراض والشكوى</span></label>
                             <textarea 
                                className="textarea bg-stone-50 h-32 text-right font-bold text-stone-700 leading-relaxed border-transparent focus:bg-white focus:ring-4 focus:ring-primary/10 transition-all rounded-2xl"
                          ...
                          <div className="form-control">
                             <label className="label py-1"><span className="label-text font-bold text-stone-500 text-xs text-right w-full">التشخيص النهائي</span></label>
                             <input 
                                type="text" 
                                className="input bg-stone-50 text-right font-black text-stone-800 border-transparent focus:bg-white focus:ring-4 focus:ring-primary/10 transition-all rounded-2xl h-14 px-6"
                  ...
                  <div className="space-y-8">
                    <Card className="p-8">
                       <h4 className="text-xs font-black text-stone-400 uppercase tracking-[0.2em] mb-6">الوصفة الطبية الإلكترونية</h4>
                       <div className="space-y-4">
                          {prescriptionItems.map((item, index) => (
                            <div key={index} className="flex gap-2 items-end group">
                               <div className="flex-1 space-y-1">
                                  {index === 0 && <label className="text-[9px] font-bold text-stone-400 block mr-2">الدواء والجرعة</label>}
                                  <div className="join w-full">
                                    <input 
                                       type="text" 
                                       placeholder="اسم الدواء" 
                                       className="input input-sm join-item bg-stone-50 w-2/3 text-right" 
                                       value={item.drugName}
                                    ...
                                    <input 
                                       type="text" 
                                       placeholder="الجرعة" 
                                       className="input input-sm join-item bg-stone-50 w-1/3 text-right text-[10px]" 
                                       value={item.dosage}
                               ...
                               <div className="w-16 space-y-1">
                                  {index === 0 && <label className="text-[9px] font-bold text-stone-400 block mr-1">الكمية</label>}
                                  <input 
                                     type="number" 
                                     className="input input-sm bg-stone-50 w-full text-center font-bold" 
                                     value={item.quantity}
                               ...
                               <button 
                                  ...
                                  className="btn btn-ghost btn-sm btn-square text-stone-300 hover:text-rose-500"
                                  disabled={prescriptionItems.length === 1}
                               >✕</button>
                            </div>
                          ))}
                          <Button variant="ghost" onClick={addPrescriptionRow} className="btn-block h-10 text-primary text-xs border-2 border-dashed border-stone-100">+ إضافة دواء آخر</Button>
                       </div>
                    </Card>
        ...
        <div className="lg:col-span-4">
           {isEncountering ? (
              <div className="animate-in slide-in-from-left-5 duration-500">
                <h2 className="text-xs font-black text-stone-900 uppercase tracking-widest mb-6 flex items-center gap-2">
              ...
              <div>
                 <div className="flex justify-between items-end mb-6">
                    <h2 className="text-xs font-black text-stone-900 uppercase tracking-widest">قائمة الانتظار</h2>
                    <Badge>{waitingList.length} مريض</Badge>
                 </div>
                 
                 <Card className="p-0 overflow-hidden">
                    <div className="divide-y divide-stone-50 max-h-[600px] overflow-y-auto scrollbar-hide">
                       {waitingList.length === 0 ? (
                          <div className="p-12 text-center opacity-30 font-bold italic">القائمة فارغة</div>
                       ) : (
                          waitingList.map((item) => (
                            <div key={item.id} className="p-6 hover:bg-stone-50 transition-colors flex items-center gap-4 group cursor-pointer" onClick={() => callPatient(item.id)}>
                               <div className="w-10 h-10 bg-stone-100 rounded-xl flex items-center justify-center font-black text-stone-400 group-hover:bg-primary group-hover:text-white transition-all">
                                 {item.queueNumber}
                               </div>
                               <div className="flex-1">
                                  <h4 className="font-bold text-stone-800 text-sm">{item.patient.user.fullName}</h4>
                                  <p className="text-[9px] font-bold text-stone-400 mt-1 uppercase tracking-widest">{item.patient.nationalId}</p>
                                </div>
