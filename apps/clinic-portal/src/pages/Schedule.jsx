    ...
    <Layout>
      {!currentClinicId ? (
        <Card className="p-20 text-center flex flex-col items-center">
           ...
           <h3 className="text-xl font-black text-content mb-4">الرجاء تحديد مقر العمل الحالي</h3>
            ...
            <div className="flex flex-col md:flex-row gap-6 items-end justify-between">
              <div className="flex gap-4 flex-1">
                <div className="flex-1 space-y-2">
                   <label className="text-[10px] font-black text-content-muted uppercase tracking-widest mr-2">التاريخ</label>
                   <input 
                     type="date" 
                     className="input bg-surface-bg w-full" 
                     value={selectedDate}
                ...
                {user?.role === 'RECEPTIONIST' && (
                  <div className="flex-1 space-y-2">
                     <label className="text-[10px] font-black text-content-muted uppercase tracking-widest mr-2">الطبيب</label>
                     <select 
                       className="select bg-surface-bg w-full rounded-2xl font-bold border-none h-14"
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              ...
              <Card className="p-20 text-center flex flex-col items-center">
                 ...
                 <p className="text-content-subtle font-black uppercase tracking-widest">لا توجد مواعيد محجوزة لهذا اليوم</p>
              </Card>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                {appointments.map((app) => (
                  (() => {
                    ...
                    return (
                  <Card key={app.id} className="p-0 overflow-hidden border border-border-main hover:border-primary/30 transition-all group">
                     <div className="flex items-stretch">
                        {/* Time Block */}
                        <div className="w-32 bg-surface-bg flex flex-col items-center justify-center border-l border-border-main group-hover:bg-primary/5 transition-colors">
                           <span className="text-xl font-black text-content">{formatArabicTime(app.startTime)}</span>
                           <span className="text-[10px] font-bold text-content-muted uppercase tracking-tighter">بداية الموعد</span>
                        </div>
                        ...
                        <div className="flex-1 p-6 flex flex-col md:flex-row justify-between items-center gap-6">
                           <div className="flex items-center gap-6">
                              <div className="avatar">
                                 <div className="w-14 h-14 rounded-2xl ring ring-surface-bg">
                                    <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${app.patient.user.fullName}`} alt="avatar" />
                                 </div>
                              </div>
                              <div>
                                 <h3 className="text-lg font-black text-content">{app.patient.user.fullName}</h3>
                                 <p className="text-xs font-bold text-content-muted uppercase">رقم وطني: {app.patient.nationalId}</p>
                              </div>
                           </div>
                           ...
                           <div className="flex items-center gap-8">
                              {user?.role === 'RECEPTIONIST' && (
                                <div className="text-right hidden md:block">
                                   <p className="text-[10px] font-black text-content-muted uppercase tracking-widest">الطبيب المعالج</p>
